...
法\upcite{Sand2004}：首先，在初始参照帧中提取出不同尺度下的分
层 Harris 角点\upcite{lucas1981iterative,noble1989descriptions}；其次，基于最小差
值平方和（Sum of Squared Differences, SSD）匹配，计算每个特征点从参照帧到其他帧的流向
量；随后，再利用局部 Lucas-Kanade 算法\upcite{lucas1981iterative,shi1998motion}将
...
接下来，文献 \cite{liu2005motion} 对上一步得到的流场使用 graph
cuts 算法\upcite{Boykov2001}将每帧图像分割成 $K$ 组时空连续的动作层，这些动作层的
...
\begin{figure}[htbp]
  ...
  \subfloat[直接放大 (c) 中秋千的动作层得到的结果（出现了黑坑）]{%
...
欧拉影像放大技术的第一步是对视频序列进行空间滤波，以得到不同的空间频率的基带。其
原因有如下两点：
